    ...
    >
      ...
      >
        <div className="flex items-center justify-between text-[11px] uppercase tracking-widest text-[#78746c] font-medium pb-4 border-b border-[#1c1b18]/6">
          ...
          <span className="font-mono text-[10px] text-[#9a958b]">Press [M]</span>
        </div>
        ...
        <p className="mt-4 text-xs sm:text-sm text-[#57534c] leading-relaxed font-normal whitespace-pre-line">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-[#78746c]">
          {musicData.tags.map((tag, idx) => (
            <React.Fragment key={tag}>
              <span>{tag}</span>
              {idx < musicData.tags.length - 1 && <span aria-hidden="true" className="text-[#bbb6ab]">·</span>}
            </React.Fragment>
          ))}
        </div>
          ...
          <div className="space-y-1.5">
            {musicData.featuredWorks.slice(0, 2).map((item, idx) => (
              <div key={idx} className="flex items-baseline justify-between text-xs">
                <span className="font-medium text-[#2d2a26] truncate max-w-[240px]">
                  {item.title}
                </span>
